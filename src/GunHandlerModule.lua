...
ParticleEffects = {
	["Blood"] = {
		PrimaryParticle = {
			...
			Size = NumberSequence.new({
				NumberSequenceKeypoint.new(0, 0.8999999761581421, 0),
				NumberSequenceKeypoint.new(1, 0.8999999761581421, 0),
			}),
		},
	},
	["Sparks"] = {
		PrimaryParticle = {
			...
			Size = NumberSequence.new({
				NumberSequenceKeypoint.new(0, 0.18750011920928955, 0.12499988079071045),
	...
	["Bullet"] = {
		...
		SecondaryParticle = {
			...
			Size = NumberSequence.new({
				NumberSequenceKeypoint.new(0, 0.30000001192092896, 0),
				NumberSequenceKeypoint.new(1, 0.30000001192092896, 0),
			}),
		},
		PointLight = {
			Enabled = false,
...
local function RecursiveClone(obj)
	...
	for k, v in pairs(obj) do
		res[RecursiveClone(k)] = RecursiveClone(v)
	end
	return res
end
...
function Module:InitServer()
	local FireGun = Instance.new("RemoteEvent", game.ReplicatedStorage)
	FireGun.Name = "FireGun"
	local GiveAmmo = Instance.new("RemoteEvent", game.ReplicatedStorage)
	GiveAmmo.Name = "GiveAmmo"
	local LogPlayer = Instance.new("RemoteEvent", game.ReplicatedStorage)
	LogPlayer.Name = "LogPlayer"
	local RequestParticleInfo = Instance.new("RemoteFunction", game.ReplicatedStorage)
	RequestParticleInfo.Name = "RequestParticleInfo"
		...
		return {
			...
			Remove = function()
				for i, v in pairs(FX) do
					game.Debris:AddItem(v, Dist.Min * 0.1)
				end
			end,
		}
	end
	...
	local function MakeSound(id, Where, volume, PlayBackSpeed)
		...
		s:Play()
		task.delay(s.TimeLength * s.PlaybackSpeed, function()
			s:Destroy()
		end)
		--s:Destroy()
	end
	...
	local function Cone(num, cone, shotty)
		if shotty == nil or shotty == false then
			return 0
		end
		if ti >= num + cone / 10 then
			ti = 0 - cone / 10
		end
		ti = ti + 0.1
		num = num + ti
	...
	local function GunMain(Player, Where, UnitRay, Right, Particle, GraphicSetting, AmmoCount)
		...
		if UnitRay == nil then
			MakeSound(SoundReference.reload, Where, 0.2, (math.random(97, 102) / 100))
			Tracker.Ammo = Tracker.MaxAmmo
			return
		end
		ti = 0 - (Tracker.BulletSpread + 0.3)
		Tracker.Ammo = Tracker.Ammo - Tracker.Burst
		if Tracker.Ammo > AmmoCount or os.clock() - Tracker.LastFired < Tracker.FireRate then
			error("ParticleGun: Sanity check didn't check out")
		end
		...
		for i = 1, Tracker.Burst do
			coroutine.wrap(function()
				...
				local length = 2000
				local ray = Ray.new(
					Where.Parent.Head.Position,
					(UnitRay.Direction + (Right * Cone(2, Tracker.BulletSpread, Tracker.Shotgun))) * length
				)
				local findparts, hitposition =
					workspace:FindPartOnRayWithIgnoreList(ray, Player.Character:GetDescendants())
				...
				if hitposition then
					if
						findparts and findparts.Parent:FindFirstChild("Humanoid")
						or findparts and findparts.Parent.Parent:FindFirstChild("Humanoid")
					then
	...
	local RequestParticleInfoF = function(Player, Particle)
		if ParticleEffects[Particle] then
			return ParticleEffects[Particle].LocalSettings
		end
	end
...
function Module:initClient(Particle, Tool)
	...
	local function CreateAnimation(id, Where)
		...
		if id == nil then
			error("ParticleGun: Animations cannot be nil!")
		end
	...
	local function CreateSound(id, Where)
		...
		if id == nil then
			error("ParticleGun: Sounds cannot be nil!")
		end
		Sound.SoundId = id
		return Sound
	end
	...
	local function CreateGUI(Setting)
		...
		ReticleImage.Size = UDim2.new(0, 50, 0, 50)
		ReticleImage.Visible = true
		local AmmoText = Instance.new("TextLabel", AmmoCounter)
		AmmoText.Size = UDim2.new(0.096, 0, 0.086, 0)
		AmmoText.Position = UDim2.new(0.522, 0.488)
		AmmoText.BackgroundTransparency = 1
		AmmoText.Visible = true
		AmmoText.Text = "10|40"
		AmmoText.FontSize = Enum.FontSize.Size14
		AmmoText.TextScaled = true
		AmmoText.TextWrap = true
		AmmoText.Font = Enum.Font.SourceSans
		AmmoText.TextStrokeTransparency = 0.4000000059604645
		AmmoText.TextWrapped = true
	...
	local Reload = function()
		if Setting.Ammo == Setting.MaxAmmo or Reloading then
			return
		end
		if Setting.Stored == 0 then
			return
		end
		Reloading = true
		Firing = false
		...
		game.ReplicatedStorage.FireGun:FireServer(
			...
			0,
			nil,
			GameSettings.SavedQualityLevel.Value,
			Setting.Ammo
		)
	...
	local Fire = function()
		...
		if Reloading == false then
			...
			if Setting.Ammo > 0 then
				...
				LocalPlayer.PlayerGui.main.Ammo.TextLabel.Text = tostring(Setting.Ammo)
					.. "|"
					.. tostring(Setting.Stored)
				--local length = 500
				...
				local UnitRay = Camera:ScreenPointToRay(
					(InputPosition.X + 10) + math.random(1, Spread) / 10,
					(InputPosition.Y + 10) + math.random(1, Spread) / 10,
					InputPosition.Z + math.random(1, Spread) / 10
				)
				game.ReplicatedStorage.FireGun:FireServer(
					Setting.Tool,
					UnitRay,
	...
	Setting.Tool.Equipped:Connect(function()
		...
		InputChanged = UIS.InputChanged:Connect(function(i, t)
			if i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch then
				...
				LocalPlayer.PlayerGui.main.HipFire.Reticle.Position =
					UDim2.new(0, i.Position.X - 25, 0, i.Position.Y - 25)
				LocalPlayer.PlayerGui.main.Ammo.TextLabel.Position =
					UDim2.new(0, i.Position.X + 15, 0, i.Position.Y - 25)
			end
		end)
