			...
			if ammo > 0 then
				if parent:FindFirstChild("AnimationController") then
					if gunanimfire == nil then
						error("GunHandler: Fire animation not found in ReplicatedStorage.Assets.Animations")
					end
				...
				local unitRay = camera:ScreenPointToRay((position.X + 1)+ math.random(1, spread)/10, position.Y + math.random(1, spread)/10 , position.Z + math.random(1, spread)/10)
